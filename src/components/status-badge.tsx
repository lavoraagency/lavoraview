...
const statusMap: Record<string, { label: string; className: string }> = {
  ...
  "facebook_48h_waiting": { label: "Facebook 48h Waiting", className: "bg-blue-100 text-blue-700 border-blue-200" },
  active: { label: "Active", className: "bg-[#1877F2] text-white border-[#1565C0]" },
  "facebook account": { label: "Facebook Account", className: "bg-[#1877F2] text-white border-[#1565C0]" },
  ...
  return (
    <span className={cn("inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border", config.className, className)}>
      <span className={cn("w-1.5 h-1.5 rounded-full mr-1.5",
        ...
        key === "48h_waiting" || key === "facebook_48h_waiting" ? "bg-blue-500" :
        key === "active" || key === "facebook account" ? "bg-white" :
